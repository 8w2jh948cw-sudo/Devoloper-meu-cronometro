...
let db;
let ui = { tab: 'timers', timerView: 'timers', modal: null, popover: null, modelsEditing: false, historyQuery: '', historyModel: 'all', historyDate: '', devCopied: null };
const BUILD = window.APP_BUILD || {developer:false,version:'0.3.0',designDefaults:{colors:{},sizes:{},shadow:{},layout:{},themePresets:[]}};
...
function shell(content,tab=ui.tab){
  const tabs=[['timers','timers','Cronômetros'],['history','history','Histórico'],['stats','stats','Estatísticas'],['settings','settings','Ajustes'],...(BUILD.developer?[['developer','sliders','Desenvolvedor']]:[])];
  return `<div class="app-shell">${content}</div><nav class="tabbar ${BUILD.developer?'developer-tabbar':''}" aria-label="Navegação principal">
...
function renderSettings(){
  ...
  return shell(`<header class="topbar simple"><h1>Ajustes</h1></header><main class="settings-content"><h2 class="settings-title">Ajustes</h2>
    ...
    <section class="settings-section"><div class="settings-card"><div class="settings-row"><span>Versão</span><span class="secondary-value">${esc(BUILD.version||'0.3.0')}</span></div></div></section>
  </main>`);
}

const DEV_SCHEMA = [
  ['Cores — tema claro','colors',[['lightBg','Fundo','color'],['lightCard','Cartões','color'],['lightText','Texto principal','color'],['lightSecondary','Texto secundário','color'],['lightLine','Divisórias','color'],['lightPlaceholder','Placeholder','color'],['lightUsed','Cronômetro usado','color'],['lightGlass','Vidro / botões flutuantes','text'],['lightFloatBorder','Borda flutuante','text']]],
  ['Cores — tema escuro','colors',[['darkBg','Fundo','color'],['darkCard','Cartões','color'],['darkText','Texto principal','color'],['darkSecondary','Texto secundário','color'],['darkLine','Divisórias','color'],['darkPlaceholder','Placeholder','color'],['darkUsed','Cronômetro usado','color'],['darkGlass','Vidro / botões flutuantes','text'],['darkFloatBorder','Borda flutuante','text']]],
  ['Cabeçalho e conteúdo','sizes',[['contentSide','Margem lateral','number'],['contentTop','Espaço superior do conteúdo','number'],['topbarHeight','Altura do cabeçalho','number'],['headerButton','Botões circulares','number'],['headerIcon','Ícones do cabeçalho','number'],['titleSize','Título do registro','number'],['headerCircleBorder','Contorno dos botões do topo','number']]],
  ['Cronômetros','sizes',[['timerMinHeight','Altura mínima','number'],['timerRadius','Arredondamento','number'],['timerPadY','Padding vertical','number'],['timerPadX','Padding horizontal','number'],['timerGap','Espaço interno','number'],['timerIconSize','Área do símbolo','number'],['timerIconRadius','Raio do símbolo','number'],['timerNameSize','Tamanho do nome','number'],['timerTimeSize','Tamanho do número','number'],['addHeight','Altura de Adicionar','number']]],
  ['Cartões flutuantes','sizes',[['floatingHeight','Altura','number'],['floatingRadius','Arredondamento','number'],['floatingGap','Espaço entre cartões','number'],['totalTimeSize','Tamanho do tempo total','number']]],
  ['Ajustes','sizes',[['settingsRadius','Arredondamento dos cartões','number'],['settingsRowHeight','Altura das linhas','number'],['settingsSide','Margem lateral','number'],['settingsPadX','Padding interno','number']]],
  ['Histórico','sizes',[['historyRadius','Arredondamento dos registros','number'],['historyPadY','Padding vertical','number'],['historyPadX','Padding horizontal','number'],['historyTitleSize','Tamanho do título','number']]],
  ['Outros cartões e modais','sizes',[['panelRadius','Arredondamento dos painéis','number'],['modalRadius','Arredondamento da janela inferior','number'],['sheetCardRadius','Arredondamento dos cartões da janela','number'],['borderWidth','Espessura de borda geral','number'],['iconStroke','Espessura dos ícones gerais','number'],['tabIcon','Ícones da barra inferior','number'],['tabIconStroke','Espessura dos ícones da barra','number'],['tabFont','Texto da barra inferior','number']]],
  ['Sombra flutuante','shadow',[['x','Posição X','number'],['y','Posição Y','number'],['blur','Desfoque','number'],['spread','Expansão','number'],['opacity','Opacidade principal','number'],['smallY','Segunda sombra Y','number'],['smallBlur','Segundo desfoque','number'],['smallOpacity','Segunda opacidade','number']]],
  ['Posicionamento interno','layout',[['timerNameAlign','Nome do cronômetro','select:left|center|right'],['timerTimeAlign','Tempo do cronômetro','select:left|center|right'],['totalJustify','Conteúdo do tempo total','select:flex-start|center|flex-end'],['cardAlign','Alinhamento vertical','select:start|center|end']]]
];
function devControl(group,key,label,type){
  const value=getPath(devDesign,`${group}.${key}`) ?? '';
  const copy=`<button class="dev-mini" data-dev-copy="${group}.${key}">Copiar</button><button class="dev-mini" data-dev-paste="${group}.${key}">Colar</button>`;
  if(type==='color') return `<div class="dev-row"><span>${esc(label)}</span><div class="dev-value"><input type="color" data-dev-path="${group}.${key}" value="${esc(value)}"><input class="dev-text" data-dev-path="${group}.${key}" value="${esc(value)}">${copy}</div></div>`;
  if(type.startsWith('select:')){const opts=type.slice(7).split('|');return `<div class="dev-row"><span>${esc(label)}</span><div class="dev-value"><select data-dev-path="${group}.${key}">${opts.map(o=>`<option ${String(value)===o?'selected':''}>${esc(o)}</option>`).join('')}</select>${copy}</div></div>`;}
  return `<div class="dev-row"><span>${esc(label)}</span><div class="dev-value"><input class="dev-text" type="${type==='number'?'number':'text'}" step="${['opacity','smallOpacity'].includes(key)?'0.01':'0.5'}" data-dev-path="${group}.${key}" value="${esc(value)}">${copy}</div></div>`;
}
function renderDeveloper(){
  if(!BUILD.developer) return renderSettings();
  const presetCards=(devDesign.themePresets||[]).map((p,i)=>`<div class="dev-preset"><div class="dev-row"><strong>${esc(p.name)}</strong><span class="secondary-value">${esc(p.id)}</span></div>${devControl(`themePresets.${i}`,'name','Nome','text')}${devControl(`themePresets.${i}`,'accent','Cor de destaque','color')}${devControl(`themePresets.${i}`,'action','Cor de ação / ativo','color')}</div>`).join('');
  const groups=DEV_SCHEMA.map(([title,group,fields])=>`<section class="settings-section"><h3 class="section-label">${esc(title)}</h3><div class="settings-card dev-card">${fields.map(([key,label,type])=>devControl(group,key,label,type)).join('')}</div></section>`).join('');
  return shell(`<header class="topbar simple"><h1>Desenvolvedor</h1></header><main class="settings-content"><h2 class="settings-title">Desenvolvedor</h2><p class="dev-intro">Alterações visuais são aplicadas ao vivo e ficam separadas da lógica de cronometragem.</p>${groups}<section class="settings-section"><h3 class="section-label">Temas disponíveis ao usuário</h3><div class="settings-card dev-card">${presetCards}</div><p class="section-footer">Você pode alterar tanto o tema Padrão quanto os temas predefinidos. O vermelho das ações de apagar não é editável por aqui.</p></section><section class="settings-section"><h3 class="section-label">Projeto</h3><div class="settings-card"><button class="settings-row button-row" id="devReset"><span>Redefinir para originais desta versão</span></button><button class="settings-row button-row" id="devExportDev"><span>Exportar nova versão Developer</span></button><button class="settings-row button-row" id="devExportFinal"><span>Exportar versão final</span></button></div><p class="section-footer">Ao exportar uma nova Developer, os valores atuais viram os novos originais. A versão final abre sem a aba Desenvolvedor.</p></section></main>`,'developer');
}
...
function render(){
  ...
  if(ui.tab==='developer'&&BUILD.developer) $app.innerHTML=renderDeveloper();
  if(ui.modal){
...
function bind(){
  ...
  document.querySelectorAll('[data-dev-paste]').forEach(b=>b.onclick=async()=>{const path=b.dataset.devPaste;let value=ui.devCopied?.value;try{const clip=await navigator.clipboard?.readText();if(clip)value=clip;}catch(_){}if(value==null){value=prompt('Valor para colar:','');if(value==null)return;}setPath(devDesign,path,normalizeDevValue(path,value));await persistDevDesign();render();toast('Valor colado');});
  if(byId('devReset'))byId('devReset').onclick=async()=>{if(!confirm('Redefinir todas as configurações visuais para os originais desta versão Developer?'))return;devDesign=clone(BUILD.designDefaults);await persistDevDesign();render();toast('Configurações redefinidas');};
  if(byId('devExportDev'))byId('devExportDev').onclick=()=>exportProject(true);
  if(byId('devExportFinal'))byId('devExportFinal').onclick=()=>exportProject(false);
}
...
function applyTheme(){
  ...
  const vars={
    ...
    '--content-side':`${cssNum(z.contentSide,18)}px`,'--content-top':`${cssNum(z.contentTop,14)}px`,'--topbar-height':`${cssNum(z.topbarHeight,54)}px`,'--header-button':`${cssNum(z.headerButton,40)}px`,'--header-icon':`${cssNum(z.headerIcon,22)}px`,'--title-size':`${cssNum(z.titleSize,17.5)}px`,
    ...
    '--floating-height':`${cssNum(z.floatingHeight,52)}px`,'--floating-radius':`${cssNum(z.floatingRadius,20)}px`,'--floating-gap':`${cssNum(z.floatingGap,10)}px`,'--total-time-size':`${cssNum(z.totalTimeSize,18)}px`,'--add-height':`${cssNum(z.addHeight,52)}px`,
...
function buildConfigSource(isDeveloper){
  const payload={developer:!!isDeveloper,version:isDeveloper?'0.3.0-dev':'0.3.0',buildId:`export-${Date.now()}`,designDefaults:clone(devDesign)};
...
async function init(){
  db=await openDB();data.models=await getAll('models');data.sessions=await getAll('sessions');data.settings={...data.settings,...(await getState('settings')||{}),simultaneous:'single'};data.current=await getState('current');devDesign=deepMerge(BUILD.designDefaults||{},(await getState(devStateKey()))||{});if(!(devDesign.themePresets||[]).some(p=>p.id===data.settings.colorTheme))data.settings.colorTheme='original';
